...
`--sensitive`로 등록한 Vercel 값은 `env pull` 시 실제 키가 복원되지 않습니다. 운영 화면에서 내부 평가 Riot ID를 `Refresh Riot Data`로 조회하면 서버가 운영 키를 사용해 최대 75건의 최근 90일 전적을 자동 적재하므로 로컬 키 복사가 필요하지 않습니다.
